...
export class Node {
    ...
    addNextNode (node) {
        const hasAlreadyNodeBetween = this.nextNodes.some(it => it.isBetween(this, node))
    ...
    isBetween (start, end) {
        ...
        if (coef === coefBase) {
            if (start.y < end.y && start.x < end.x) {
                return this.y > start.y && this.y < end.y && this.x > start.x && this.x < end.x
            }
            if (start.y < end.y && start.x > end.x) {
                return this.y > start.y && this.y < end.y && this.x < start.x && this.x > end.x
            }
...
export const computePart2 = items => {
    ...
    do {
        ...
        const otherAsteroids = bestAsteroid.nextNodes.filter(it => it.id !== bestAsteroid.id)
        ...
        const orderedDestroyedAsteroidOfRun = otherAsteroids.sort((ast1, ast2) => {
            const otherVector1 = new Node({x: ast1.x - bestAsteroid.x, y: bestAsteroid.y - ast1.y})
            const angle1 = getAngleFromOrigin(otherVector1)
            const otherVector2 = new Node({x: ast2.x - bestAsteroid.x, y: bestAsteroid.y - ast2.y})
