...
export const manhattanDistance = ({ x: x1, y: y1 }, { x: x2, y: y2 }) => Math.abs(x1 - x2) + Math.abs(y1 - y2)
...
export const hasCollision = (v1, v2) => {
    ...
    console.log({coefDirecteurV1, coefDirecteurV2, ordonneAOrigine1, ordonneAOrigine2})

    if (coefDirecteurV1 === coefDirecteurV2) {
        return false;
    }

    const commonX = (ordonneAOrigine2 - ordonneAOrigine1) / (coefDirecteurV1 - coefDirecteurV2)

    const onV1 = v1.x2 <= v1.x1
        ? v1.x1 >= commonX && commonX <= v1.x2
        : v1.x2 >= commonX && commonX <= v1.x1

    const onV2 = v2.x2 <= v2.x1
...
export const getManatthanFrom = (mapping1, mapping2) => {
    ...
    mapping2.forEach(command => {
        ...
        for (let i = 0; i < amount; i++) {
            ...
            const conflict = alreadyWiredPoints.find(it => it.x === point.x && it.y === point.y)
            if (conflict) {
    ...
    return Math.min(...conflictPoints.map(it => manhattanDistance(new Point({x: 0, y: 0}), it)))
}
    ...
    mapping1.forEach(command => {
        ...
        switch (direction) {
            case 'U': {
                for (let i = 0; i < amount; i++) {
                    alreadyWiredPoints.push(
                        new Point({x: initialPoint.x, y: initialPoint.y + i + 1, cost: initialPoint.cost + i + 1})
                    )
                }
                initialPoint.up(amount);
                break;
            ...
            case 'R': {
                for (let i = 0; i < amount; i++) {
                    alreadyWiredPoints.push(
                        new Point({x: initialPoint.x + (i + 1), y: initialPoint.y, cost: initialPoint.cost + i + 1})
                    )
                }
                initialPoint.right(amount);
                break;
...
export const run = async part => {
    const data = await parse(`./data/day-03-${part}.txt`);
