...
const parseMap = (
  ...
  depthFirstSearch(grid, robot, {
    enterVertex({ currentVertex, previousVertex }) {
      ...
      const result = computeIntProgramWithState(
        intProgramState,
        startPointer,
        [BigInt(direction)],
        (output, pgmState, nextPointer) => {
  ...
  return {
    ...
    iterations,
    maxDepth,
    intProgramStateWhenFound,
    startPointerWhenFound
  };
};
...
export const computePart2 = items => {
  const { grid, intProgramStateWhenFound, startPointerWhenFound } = parseMap(
    items,
    undefined,
    undefined,
    true
  );
  const oxygenRoom = grid
    .getAllPoints()
    .find(it => it.type === RESULT.OXYGEN_ROOM);
  const { maxDepth } = parseMap(
    intProgramStateWhenFound,
    startPointerWhenFound,
    oxygenRoom
  );
