...
export const findAllSequences = availableValue =>
  availableValue.reduce((acc, currentValue) => {
    const otherValues = availableValue.filter(it => it !== currentValue);
    if (otherValues.length !== 0) {
      const subSequences = findAllSequences(otherValues);
      subSequences.map(it => [currentValue, ...it]).forEach(it => acc.push(it));
    } else {
...
export const sequentialIntProgram = (intProgram, sequence) => {
  ...
  sequence.forEach(seqVal => {
    ...
    computeIntProgram(intProgram, [seqVal, output], newOut => {
      output = newOut;
    });
  });
  return output;
